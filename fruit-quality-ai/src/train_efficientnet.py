"""Train an EfficientNetB0 fruit quality classifier with TensorFlow/Keras.
...
Each split folder should contain the same 16 class folders.
"""
...
ACCURACY_PLOT_PATH = OUTPUTS_DIR / "figures" / "efficientnetb0_accuracy.png"
LOSS_PLOT_PATH = OUTPUTS_DIR / "figures" / "efficientnetb0_loss.png"
REPORT_PATH = OUTPUTS_DIR / "reports" / "efficientnetb0_training_report.txt"


def print_startup_message() -> None:
    """Print basic runtime information before training starts."""
    gpus = tf.config.list_physical_devices("GPU")
    gpu_detected = "Yes" if gpus else "No"
...
def load_dataset(split_dir: Path, shuffle: bool) -> tf.data.Dataset:
    """Load one dataset split from class folders."""
...
def prepare_dataset(dataset: tf.data.Dataset) -> tf.data.Dataset:
    """Cache and prefetch batches for smoother training and evaluation."""
...
def build_model(number_of_classes: int) -> tf.keras.Model:
    ...
    # In current tf.keras EfficientNet implementations, input rescaling is built
    # into the application model itself. Keeping raw image pixels here helps keep
    # training, evaluation, and later inference preprocessing consistent.
...
def build_report(
    ...
    lines = [
        ...
        "",
        f"Model save path: {MODEL_PATH}",
        f"Accuracy plot path: {ACCURACY_PLOT_PATH}",
...
def main() -> None:
    """Train, evaluate, and save the EfficientNetB0 model."""
    ...
    save_history_plot(
        ...
        title="EfficientNetB0 Training Accuracy",
        y_label="Accuracy",
    ...
    save_history_plot(
        ...
        title="EfficientNetB0 Training Loss",
        y_label="Loss",
